    ...
    const submitImage = async () => {
        ...
        try {
            ...
            try {
                ...
                throw new Error(`Invalid JSON response (HTTP ${res.status}): ${rawText}`);
            }

            // Validate shape
            if (!parsed || typeof parsed !== "object" || !("status" in parsed)) {
                throw new Error(`Unexpected response shape: ${rawText}`);
            }

            const result = parsed as APIResponse;
            ...
            setImageUrl(
                result.dynamicModel.imageUrl ? `data:image/jpeg;base64,${result.dynamicModel.imageUrl}` : imageUrl
            );
