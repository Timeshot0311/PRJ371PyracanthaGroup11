...
export function ImageUploader() {
  ...
  const submitImage = async () => {
    if (!uploadedImage) return toast.info("Please select an image");

    const token = authToken.get();
    if (!token) {
      return toast.error("Please log in first to analyze images.");
    }

    setIsPending(true);
    try {
      // 1) get the logged-in user profile
      const me = await getMyProfile(token); // requires Authorization: Bearer <token>
      const userId = me.UserId;

      // 2) convert image
      const base64Image = await convertToBase64(uploadedImage);

      // 3) call engine with real user id
      const apiResponse = await investigateImage(
        {
          user_id: userId,
