...
export function AnalyticsKpis() {
  ...
  const getProvince = (s: ProvinceStat) => s.province ?? s.provinces ?? "Unknown";
  const getTotals = (s: ProvinceStat) => (typeof s.totals === "number" ? s.totals : 0);
  ...
  const topProvince = stats.slice().sort((a, b) => getTotals(b) - getTotals(a))[0];
  ...
  return (
    <div className="my-20">
      <div className="grid grid-cols-1 gap-6 md:grid-cols-4">
        <KPI title="Total Detections" value={totalDetections.toLocaleString()} icon={<ScatterChart className="size-4" />} />
        <KPI title="Provinces Covered" value={provincesCovered.toString()} icon={<MapPin className="size-4" />} />
        <KPI title="Top Province" value={topProvince ? getProvince(topProvince) : "—"} icon={<Cpu className="size-4" />} />
        <KPI title="Active Users" value={activeUsersToday.toString()} icon={<TrendingUp className="size-4" />} />
      </div>
